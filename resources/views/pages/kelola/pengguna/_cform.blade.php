  ...
  <div class="col-md-6 pe-md-3 pe-lg-3">
      <div class="form-group mb-3">
          ...
          <input required type="text" name="name" class="form-control @error('name') is-invalid @enderror"
              value="{{ old('name') }}" placeholder="Masukkan nama pengguna" id="name" autocomplete="off">
          @error('name')
...
<div class="row px-1">
  <div class="col-md-6 pe-md-3 pe-lg-3">
    <div class="form-group mb-3">
      <label for="role" class="form-label mb-1">Role/Hak Akses</label>
        <select name="role" class="form-select @error('role') is-invalid @enderror" id="role" required>
          ...
          <option value="staff" {{ 'staff' == old('role') ? 'selected' : '' }}>Staff</option>
          <option value="pimpinan" {{ 'pimpinan' == old('role') ? 'selected' : '' }}>Pimpinan</option>
        </select>
        @error('role')
        <span class="invalid-feedback">{{ $message }}</span>
        @enderror
